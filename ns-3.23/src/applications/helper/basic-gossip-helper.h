 ...
 */
#ifndef BASIC_GOSSIP_HELPER_H
#define BASIC_GOSSIP_HELPER_H
 ...
 */
class BasicGossipServerHelper
{
public:
  /**
   * Create BasicGossipServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  BasicGossipServerHelper (uint16_t port);
  ...
  /**
   * Create a BasicGossipServerApplication on the specified Node.
   *
  ...
  /**
   * Create a BasicGossipServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::BasicGossipServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an BasicGossipServer will be installed.
 ...
 */
class BasicGossipClientHelper
{
public:
  /**
   * Create BasicGossipClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param ip The IP address of the remote udp echo server
   * \param port The port number of the remote udp echo server
   */
  BasicGossipClientHelper (Address ip, uint16_t port);
  /**
   * Create BasicGossipClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   ...
   * \param port The port number of the remote udp echo server
   */
  BasicGossipClientHelper (Ipv4Address ip, uint16_t port);
  /**
   * Create BasicGossipClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   ...
   * \param port The port number of the remote udp echo server
   */
  BasicGossipClientHelper (Ipv6Address ip, uint16_t port);
  ...
  /**
   * Given a pointer to a BasicGossipClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be BasicGossipClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the BasicGossipClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the BasicGossipClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::BasicGossipClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an BasicGossipClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
